...
- **The rest strip is a divider that carries a value, not a sixth column.** Rest isn't a measurement of the set, it's the gap after it, so `RestStrip` sits outside the table's column grid — the point of putting it in the flow is that you can see where the gaps are uneven. Volt marks a set carrying its own override, the only way to tell the two levels apart at a glance. **It is recessed (`bg-surface-1`) only ahead of you.** A gap whose set is ticked has been *served* — the same finished state as the row above it — so it takes the same `bg-volt-tint`, the completed row's `bg-volt-fade` already composited over `--color-bg` and therefore the identical pixel. Without that a run of completed sets read as stripes: volt row, grey gap, volt row, on the densest screen in the app. The recess survives where it still answers something live, which is the rest you have yet to take, and the strip stops snapping back to grey when the running timer's 2.5 s "Rest done" window expires. It renders after every working set including the last (that rest runs too) and never after a warm-up (those start no timer, so naming a rest would be a lie). It is 40 px, not 44: it repeats between every set on the one screen the design keeps dense, and it matches `RpePicker`'s chips, which are this app's floor for a repeated control.
